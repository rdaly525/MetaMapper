...
from metamapper.wasm.interp import convention
from metamapper.wasm.interp import log
from metamapper.wasm.interp import num
from metamapper.wasm.interp import structure
...
def exec_expr(
    ...
    while True:
        ...
        if opcode == convention.i32_eqz:
            ...
            continue
        if opcode >= convention.i32_eq and opcode <= convention.i32_ge_u:
            b = stack.pop().n
            a = stack.pop().n
            ...
            if opcode == convention.i32_ne:
                stack.add(Value.from_i32(int(a != b)))
                continue
            if opcode == convention.i32_lt_s:
                stack.add(Value.from_i32(int(a < b)))
                continue
            if opcode == convention.i32_lt_u:
                stack.add(Value.from_i32(int(num.int2u32(a) < num.int2u32(b))))
                continue
            if opcode == convention.i32_gt_s:
                stack.add(Value.from_i32(int(a > b)))
                continue
            if opcode == convention.i32_gt_u:
                stack.add(Value.from_i32(int(num.int2u32(a) > num.int2u32(b))))
                continue
            if opcode == convention.i32_le_s:
                stack.add(Value.from_i32(int(a <= b)))
                continue
            if opcode == convention.i32_le_u:
                stack.add(Value.from_i32(int(num.int2u32(a) <= num.int2u32(b))))
                continue
            if opcode == convention.i32_ge_s:
                stack.add(Value.from_i32(int(a >= b)))
                continue
            if opcode == convention.i32_ge_u:
                stack.add(Value.from_i32(int(num.int2u32(a) >= num.int2u32(b))))
                continue
            continue
        if opcode == convention.i64_eqz:
            stack.add(Value.from_i32(int(stack.pop().n == 0)))
            continue
        if opcode >= convention.i64_eq and opcode <= convention.i64_ge_u:
            b = stack.pop().n
            a = stack.pop().n
            ...
            if opcode == convention.i64_ne:
                stack.add(Value.from_i32(int(a != b)))
                continue
            if opcode == convention.i64_lt_s:
                stack.add(Value.from_i32(int(a < b)))
                continue
            if opcode == convention.i64_lt_u:
                stack.add(Value.from_i32(int(num.int2u64(a) < num.int2u64(b))))
                continue
            if opcode == convention.i64_gt_s:
                stack.add(Value.from_i32(int(a > b)))
                continue
            if opcode == convention.i64_gt_u:
                stack.add(Value.from_i32(int(num.int2u64(a) > num.int2u64(b))))
                continue
            if opcode == convention.i64_le_s:
                stack.add(Value.from_i32(int(a <= b)))
                continue
            if opcode == convention.i64_le_u:
                stack.add(Value.from_i32(int(num.int2u64(a) <= num.int2u64(b))))
                continue
            if opcode == convention.i64_ge_s:
                stack.add(Value.from_i32(int(a >= b)))
                continue
            if opcode == convention.i64_ge_u:
                stack.add(Value.from_i32(int(num.int2u64(a) >= num.int2u64(b))))
                continue
            continue
        ...
        if opcode >= convention.i32_add and opcode <= convention.i32_rotr:
            ...
            a = stack.pop().n
            if opcode in [
                convention.i32_div_s,
                convention.i32_div_u,
                convention.i32_rem_s,
                convention.i32_rem_u,
            ]:
                if b == 0:
            ...
            if opcode == convention.i32_shl:
                ...
                continue
            if opcode == convention.i32_shr_s:
                stack.add(Value.from_i32(a >> (b % 0x20)))
                continue
            if opcode == convention.i32_shr_u:
        ...
        if opcode >= convention.i64_add and opcode <= convention.i64_rotr:
            ...
            a = stack.pop().n
            if opcode in [
                convention.i64_div_s,
                convention.i64_div_u,
                convention.i64_rem_s,
                convention.i64_rem_u,
            ]:
                if b == 0:
                    raise Exception('pywasm: integer divide by zero')
            ...
            if opcode == convention.i64_shl:
                ...
                continue
            if opcode == convention.i64_shr_s:
                stack.add(Value.from_i64(a >> (b % 0x40)))
                continue
            if opcode == convention.i64_shr_u:
